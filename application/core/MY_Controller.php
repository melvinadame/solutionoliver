...
class MY_Controller extends CI_Controller {
    ...
    protected function access_only_allowed_members() {
        if ($this->access_type === "all") {
        ...
        } else if (($this->module_group === "ticket" && $this->access_type === "specific") || ($this->module_group === "lead" && $this->access_type === "own") || ($this->module_group === "client" && $this->access_type === "own")) {
            ...
            //can access if it's clients module and user has access to own clients
            return true;
        } else {
            redirect("forbidden");
        }
    ...
    protected function access_only_allowed_members_or_client_contact($client_id) {
        ...
        } else if ($this->module_group === "client" && $this->access_type === "own") {
            return true; //can access if it's clients module and user has access to own clients
    ...
    protected function access_only_allowed_members_or_contact_personally($user_id) {
        if (!($this->access_type === "all" || $this->access_type === "own" || $user_id === $this->login_user->id)) {
            redirect("forbidden");
        }
    }
